...
def actualizar_entrenamiento_completo(id):
    ...
    if 'ejercicios' not in data or not isinstance(data['ejercicios'], list):
        return jsonify({'error': 'El campo "ejercicios" es requerido y debe ser una lista.'}), 400

    for i, ejercicio_data in enumerate(data['ejercicios']):
        if not isinstance(ejercicio_data,
                          dict) or 'ejercicios_id' not in ejercicio_data or 'series' not in ejercicio_data:
            return jsonify(
                {'error': f'El ejercicio en el índice {i} debe ser un objeto con "ejercicios_id" y "series".'}), 400

        # Se valida que el ejercicio exista, pero no su pertenencia a la rutina
        ejercicio_id = ejercicio_data['ejercicios_id']
        if not isinstance(ejercicio_id, int):
            return jsonify(
                {'error': f'El campo "ejercicios_id" en el ejercicio del índice {i} debe ser un entero.'}), 400

        ejercicio = Ejercicio.query.get(ejercicio_id)
        if not ejercicio:
            return jsonify({'error': f'El ejercicio con ID {ejercicio_id} no existe.'}), 404

        if not isinstance(ejercicio_data['series'], list) or not ejercicio_data['series']:
            return jsonify(
                {'error': f'El campo "series" del ejercicio en el índice {i} debe ser una lista no vacía.'}), 400

        for j, serie_data in enumerate(ejercicio_data['series']):
            if not isinstance(serie_data, dict) or 'repeticiones' not in serie_data or 'peso_kg' not in serie_data:
                return jsonify({
                                   'error': f'La serie en el índice {j} del ejercicio {i} debe contener "repeticiones" y "peso_kg".'}), 400

    try:
        if 'fecha' in data:
            entrenamiento.fecha = data['fecha']

        EntrenamientoRealizado.query.filter_by(entrenamientos_id=id).delete()

        for ejercicio_data in data['ejercicios']:
            entrenamiento_realizado = EntrenamientoRealizado(
                entrenamientos_id=id,
                ejercicios_id=ejercicio_data["ejercicios_id"]
            )
            db.session.add(entrenamiento_realizado)
            db.session.flush()
        ...
        db.session.commit()
        return jsonify({'mensaje': 'Entrenamiento completo actualizado con éxito'}), 200

    except SQLAlchemyError as e:
        db.session.rollback()
        return jsonify({'error': 'Error en la base de datos al actualizar', 'detalle': str(e)}), 500
